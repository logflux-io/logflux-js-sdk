...
# Publish LogFlux JS SDK to a local public repository directory.
# Extracts sdks/javascript/ from the monorepo, squashes into a single clean commit,
# and syncs to the local public repo.
#
# The public repo lives at ../logflux-public/logflux-js-sdk/.
# From there you can push to GitHub manually.
#
# Usage:
#   ./scripts/publish.sh [--dry-run] [--tag v3.0.0] [--push]
#
# Prerequisites:
...
PUBLIC_DIR="$(dirname "$MONO_ROOT")/logflux-public/logflux-js-sdk"
WORK_DIR="$(mktemp -d)"
...
COMMIT_MSG="LogFlux JavaScript SDK"
if [ -n "$TAG" ]; then
    COMMIT_MSG="LogFlux JavaScript SDK $TAG"
fi
...
if [ -d "$PUBLIC_DIR/.git" ]; then
    ...
    git remote add origin git@github.com:logflux-io/logflux-js-sdk.git
fi
